...
type Collector = [
  ...
  centerX: number,
  centerY: number,
  nearestPoints: Array<CollectorPixel>,
];

export function createCanvasFragments(targetCanvas: HTMLCanvasElement, rng: RandomNumberGenerator, desiredSize?: number): Array<CanvasFragment> {
  ...
  const sprites: Array<CanvasFragment> = [];
  ...
  const processedPixels: Array<boolean> = Array(width * height);
  let addedAtLeastOne = false;

  function addPixelToCollector(x: number, y: number, collector: Collector) {
    if (x < 0 || y < 0 || x >= width || y >= height) return;

    const arrayIndex = y * width + x;
    if (processedPixels[arrayIndex]) return;
    processedPixels[arrayIndex] = true;
    if (!pixelArray[arrayIndex * 4 + 3]) return;
    addedAtLeastOne = true;
    collector[COLLECTOR_MIN_X] = Math.min(
      x,
      collector[COLLECTOR_MIN_X]
    );
    collector[COLLECTOR_MAX_X] = Math.max(
      x,
      collector[COLLECTOR_MAX_X]
    );
    collector[COLLECTOR_MIN_Y] = Math.min(
      y,
      collector[COLLECTOR_MIN_Y]
    );
    collector[COLLECTOR_MAX_Y] = Math.max(
      y,
      collector[COLLECTOR_MAX_Y]
    );
  ...
  collectors
    .sort((a: Collector, b: Collector) => b[COLLECTOR_NEAREST].length - a[COLLECTOR_NEAREST].length)
    .map((collector) => {
